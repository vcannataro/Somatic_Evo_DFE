...
x.max.distance <- 12  #mutation state space will go from 0 to this number 
...
# par(mar=c(5,7,4,5)+.1)
# barplot(mut.dist.mat,horiz=F,main="Human, differentiation rate scenario",axes=T,axisnames=T,names.arg=year.vec,xlab="Age (Years)",col=rainbow(iter),ylab="Probability each mutation caused the tumor")
...
density.plotter <- function(expdiff,xlim1,xlim2,ylim1,ylim2,extent){
  ...
  if(expdiff==1){
    ...
    par(mfrow =c(1,1))
    plot(post3.new, x=mut.space, type="l", lwd=6,xlim=c(xlim1,xlim2),ylim=c(ylim1,ylim2),
         ylab="Density", xlab="Differentiation Rate (days)",main="Probability densities of differentiation rate given fixed mutations in humans",col="dark green", cex.lab=1.5, cex.axis=1.5,lty=2);abline(v=lambda.0,col="red",lwd=3)
    if(extent != 1){
      for(i in 2:extent){
...
#xlab=expression(paste("Differentiation Rate, ", nu)) actual nu here<--- 
# 
...
human.expdiff.meannu <- rep(NA,iter)

for(j in 1:iter){
  human.expdiff.meannu[j] <- expected.finder(post.den.full.expdiff[[j]],mut.space)
}
