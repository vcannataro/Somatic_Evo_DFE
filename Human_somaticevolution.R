...
iter <- 20 #number of mutations to calculate (and lambda densities for each mutation). This is the counter you wait for when running the code
...
density.plotter <- function(expdiv,powdiv,xlim1,xlim2,ylim1,ylim2,extent){
  if(expdiv==1){
    ...
    par(mfrow =c(1,1))
    #plot(post1.new,type="l",x=mut.space)
    plot(post1.new, type="l",x=mut.space, lwd=6,xlim=c(xlim1,xlim2),ylim=c(ylim1,ylim2),
         ylab="Density", xlab="Division Rate (days)",main="Probability densities of division rate given fixed mutations in humans",col="dark green",lty=2, cex.lab=1.5, cex.axis=1.5)
...
# density.plotter(1,0,xlim1=0,xlim2=.4,ylim1=0,ylim2=85,8)
# abline(v=nu.0,lwd=3,col="red")
# abline(v=0);abline(h=0)
# 
# density.plotter(1,0,xlim1=0.31,xlim2=.33,ylim1=0,ylim2=.03,8)
# abline(v=nu.0,lwd=3,col="red")
# abline(v=0);abline(h=0)
...
# plot((1-no.cancer.vec.powerdiv)*1e2,lwd=5,type="l", xlab="Age (years)", ylab="Percent of the population with 
...
# par(mar=c(4,7,4,5)+.1)
# options(scipen=10)
# plot(prob.cancer.powerdiv[1:8], xlab="Fixed Mutation Number",ylab="Probability of Tumorigenesis in a Crypt",type="b", lwd=4,cex.lab=1.5, cex.axis=1.5,ylim=c(0,0.001),main="Probability of Tumorigenesis in a Human Crypt",lty=3)
# lines(prob.cancer.expdiv[1:8], xlab="Fixed Mutation Number",ylab="Probability of Tumorigenesis in a crypt",type="b", lwd=4,cex.lab=1.5, cex.axis=1.5,lty=2)
...
expected.vec.humanexp <- rep(NA,iter)
for(i in 1:iter){
  expected.vec.humanexp[i] <- expected.finder(post.den.expdiv[[i]],mut.space)
}



total.probcancer <- rep(NA,iter)
mut.dist.mat <- matrix(nrow=iter,ncol=length(year.vec))
for(j in 1:length(year.vec)){
  for(i in 1:iter){
